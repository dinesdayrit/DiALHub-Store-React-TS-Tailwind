  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="relative w-full max-w-lg rounded-lg bg-white p-8 shadow-xl">
        ...
        <h1 className="mb-4 text-2xl font-semibold text-gray-800">Shopping Cart</h1>
        {cartItems.map((item) => (
          <div key={item.id} className="flex items-center border-b border-gray-200 py-4">
            ...
            <div className="ml-4 flex-1">
              ...
              <div className="mt-2 flex items-center space-x-2">
                <button className="p-1 text-gray-600 hover:text-gray-800">Remove</button>
              </div>
            </div>
            <div className="flex items-center">
              <button className="h-8 w-8 border border-gray-200 text-gray-600">-</button>
              <span className="mx-2">1</span>
              <button className="h-8 w-8 border border-gray-200 text-gray-600">+</button>
            </div>
            <p className="ml-4 text-lg font-medium text-gray-800">${item.price.toFixed(2)}</p>
          </div>
        ))}
